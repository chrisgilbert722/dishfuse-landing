...
/**
 * DishFuse — Landing (Navy + Gold) — Safe Auto-Optimize + ROI Calculator
 * - No sections removed; only safe perf improvements
 * - Smart video preload after first paint (poster + fade-in)
 * - Calculator directly under Hero (Premium Glass + Gold)
 * - Inputs: Monthly Revenue, Food Cost %, Monthly Waste
 * - Outputs: 3 KPI cards (Hybrid wording) + CTA + mini guarantee
 * - Auto currency detect (+ selector to override)
 * - Trust badges: PNGs (aws.png, stripe.png, encryption.png, ai.png, support.png)
 */
...
// 720p variants for mobile (safe optimization)
const HERO_MOBILE =
...
// Helper: pick a reasonable default currency from locale
...
export default function LandingPage() {
  ...
  const [loadVideos, setLoadVideos] = useState(false); // smart preload after first paint
  ...
  useEffect(() => {
    ...
    // Smart preload videos after first paint (poster first + fade-in)
    requestAnimationFrame(() => {
      setTimeout(() => setLoadVideos(true), 300);
    });
  ...
  return (
    <main className="min-h-screen text-white bg-gradient-to-b from-[#1F2D50] to-[#24335D]">
        ...
        * {
        ...
        .gold-glow::before {
          ...
          background: radial-gradient(60% 60% at 50% 50%,
            rgba(244,199,98,0.22) 0%, rgba(244,199,98,0.12) 35%, transparent 70%);
        ...
        .btn {
          ...
          font-size: clamp(14px, 1.8vw, 16px); line-height: 1.1; transition: all .2s ease;
        ...
        .btn-ghost { border: 2px solid rgba(255,255,255,.85); color:#fff; padding: 10px 16px; }
        ...
        .priceCard.popular { border: 1px solid rgba(244,199,98,.65); box-shadow: 0 18px 40px rgba(244,199,98,.1);
          background: radial-gradient(1200px 400px at 20% -10%, rgba(244,199,98,.12), transparent), linear-gradient(160deg, #102042, #0b1222);
        }
        ...
        .faq-q { display:flex; width:100%; justify-content:space-between; align-items:center; gap:12px; padding:16px 18px; border-radius:14px; background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.08); transition:background .2s, border .2s; }
      ...
      <section className="relative min-h-[84vh] flex items-center overflow-hidden">
        {/* Poster shows instantly, video loads after first paint */}
        <img
          src="/poster-hero.jpg"
          alt="Chef in the kitchen"
          className={`absolute inset-0 w-full h-full object-cover opacity-60 fade-in ${loadVideos ? "loaded" : ""}`}
          width={1920}
          height={1080}
          loading="eager"
          decoding="async"
        ...
        {loadVideos && (
          <video
            ...
            className={`absolute inset-0 w-full h-full object-cover opacity-0 ${loadVideos ? "fade-in loaded" : ""}`}
      ...
      {/* 🔥 ROI PROFIT CALCULATOR — Premium Glass Card */}
      <ROIProfitCalculator />
      ...
      <section id="demo" className="relative section overflow-hidden">
        ...
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover opacity-50 fade-in ${loadVideos ? "loaded" : ""}`}
          width={1920}
          height={1080}
          loading="lazy"
          decoding="async"
        ...
        <p className="lead mb-10">Choose the plan that fits your restaurant. Cancel anytime — no contracts.</p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              { plan: "Custom", price: "Let’s Talk", desc: "Tailored solutions for multi-brand & franchise groups.", features: ["Dedicated manager", "Custom integrations", "Data migration", "Team training"], popular: false },
            ].map((t) => (
              <div key={t.plan} className={`card priceCard ${t.popular ? "popular" : ""}`}>
                {t.popular && (
                  <div className="mb-3 text-xs font-extrabold text-[#0B1222]"
            ...
            {[
              ...
              <div key={t.name} className="glass rounded-2xl p-6 border border-white/10">
                <div className="flex items-center gap-4 mb-4">
                  <img src={t.img} alt={t.name} className="w-14 h-14 rounded-2xl object-cover" width={56} height={56} loading="lazy" decoding="async" />
                  <div>
      ...
      <section id="cta" className="section">
        <div className="container">
          <div className="glass rounded-3xl p-10 md:p-14 text-center">
            ...
            <a href="#pricing" className="btn btn-primary gold-glow">Start Free 14-Day Trial</a>
            <div className="text-white/70 text-xs mt-2">Cancel anytime — zero risk</div>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-white/10 py-10 bg-[#0A1120]">
        <div className="container flex flex-col items-center gap-4">
          <img src={LOGO_FOOTER} alt="DishFuse" className="h-6 md:h-7" width={160} height={40} loading="lazy" decoding="async" />
...
/* ---------- ROI PROFIT CALCULATOR (Premium Glass + Gold) ---------- */
/* Placement: directly under Hero */
function ROIProfitCalculator() {
  // Inputs
  const [revenue, setRevenue] = useState(100000); // default for attention (converts well)
  const [foodPct, setFoodPct] = useState(32);
  const [waste, setWaste] = useState(2500);

  // Currency
  const [currency, setCurrency] = useState(detectCurrency());
  const currencyLocales = {
    USD: "en-US", GBP: "en-GB", EUR: "de-DE", CAD: "en-CA", AUD: "en-AU", NZD: "en-NZ",
    MXN: "es-MX", BRL: "pt-BR", INR: "en-IN", JPY: "ja-JP", CNY: "zh-CN", HKD: "zh-HK",
    SGD: "en-SG", ZAR: "en-ZA", SEK: "sv-SE", NOK: "nb-NO", CHF: "de-CH",
  };
  const fmt = React.useMemo(
    () => new Intl.NumberFormat(currencyLocales[currency] || "en-US", {
      style: "currency", currency, maximumFractionDigits: ["JPY"].includes(currency) ? 0 : 2
    }),
    [currency]
  );

  // Simple, believable model:
  // - Recover ~35% of declared waste
  // - Improve food spend by ~5% (of food spend)
  const foodSpend = revenue * (foodPct / 100);
  const savingsFromWaste = waste * 0.35;
  const savingsFromFood = foodSpend * 0.05;
  const addedMonthlyProfit = Math.max(0, savingsFromWaste + savingsFromFood);
  const marginImprovementPct = revenue > 0 ? (addedMonthlyProfit / revenue) * 100 : 0;
  const addedAnnualProfit = addedMonthlyProfit * 12;

  // Animate numbers on appear (IntersectionObserver)
  ...
  const monthlyTick = useTicker(addedMonthlyProfit, animate);
  const annualTick = useTicker(addedAnnualProfit, animate);
  const marginTick = useTicker(marginImprovementPct, animate);

  // Helpers for controlled inputs
  const clamp = (v, min, max) => Math.min(max, Math.max(min, v));

  return (
    <section id="calculator" ref={ref} className="section relative overflow-hidden">
        ...
        <div className="glass rounded-3xl p-6 md:p-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-6 mb-6">
            <div>
              ...
              <p className="lead">Move three sliders — watch your profit meter fill.</p>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6 md:gap-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  id="rev" type="range" min={10000} max={200000} step={1000}
                  value={revenue}
                  onChange={(e) => setRevenue(clamp(parseInt(e.target.value || "0", 10), 10000, 200000))}
                  className="w-full accent-[#f4c762]"
                />
                <div className="flex justify-between text-xs text-white/60 mt-1">
                  <span>{fmt.format(10000)}</span><span>{fmt.format(200000)}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  id="food" type="range" min={20} max={45} step={1}
                  value={foodPct}
                  onChange={(e) => setFoodPct(clamp(parseInt(e.target.value || "0", 10), 20, 45))}
                  className="w-full accent-[#f4c762]"
                />
                <div className="flex justify-between text-xs text-white/60 mt-1">
              ...
              <div>
                <div className="mb-1 flex items-center justify-between">
                  <label htmlFor="waste" className="font-semibold">Waste per month</label>
                  <span className="text-white/70 text-sm">{fmt.format(waste)}</span>
                </div>
                <input
                  id="waste" type="range" min={0} max={10000} step={100}
                  value={waste}
                  onChange={(e) => setWaste(clamp(parseInt(e.target.value || "0", 10), 0, 10000))}
                  className="w-full accent-[#f4c762]"
                />
                <div className="flex justify-between text-xs text-white/60 mt-1">
                  <span>{fmt.format(0)}</span><span>{fmt.format(10000)}</span>
                </div>
              </div>

              <p className="text-xs text-white/60">Tip: If you don’t track waste yet, start with {fmt.format(2000)}–{fmt.format(4000)}.</p>
            </div>

            {/* Results */}
            <div className="space-y-4">
              {/* KPI Cards — 3 metrics (Hybrid wording) */}
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="card text-center">
                  <div className="text-xs text-white/70">Added profit</div>
                  <div className="text-xl md:text-2xl font-extrabold" style={{ color: "var(--gold)" }}>
                    {fmt.format(monthlyTick)}
                  </div>
                  <div className="text-xs text-white/70">/ month back in profits</div>
                </div>
                <div className="card text-center">
                  <div className="text-xs text-white/70">Margin lift</div>
                  <div className="text-xl md:text-2xl font-extrabold" style={{ color: "var(--gold)" }}>
                    {marginTick.toFixed(1)}%
                  </div>
                  <div className="text-xs text-white/70">added to margins</div>
                </div>
                <div className="card text-center">
                  <div className="text-xs text-white/70">Annual impact</div>
                  <div className="text-xl md:text-2xl font-extrabold" style={{ color: "var(--gold)" }}>
                    {fmt.format(annualTick)}
                  </div>
                  <div className="text-xs text-white/70">/ year projected</div>
                </div>
              </div>

              <div className="text-xs text-white/60">Estimates assume moderate waste recovery & menu optimization. Results vary by concept & compliance.</div>

              {/* CTA + mini guarantee */}
              <div className="flex flex-wrap items-center gap-3">
                <a href="#pricing" className="btn btn-primary gold-glow">Start Free 14-Day Trial</a>
                <div className="text-white/70 text-xs">Cancel anytime — zero risk</div>
              </div>
            </div>
          </div>
        </div>
